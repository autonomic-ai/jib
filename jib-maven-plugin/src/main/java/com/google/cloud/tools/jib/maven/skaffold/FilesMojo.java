...
public class FilesMojo extends SkaffoldBindingMojo {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        // where
        // the plugin is explicitly configured (even though _skaffold-files is a jib-maven-plugin
        // goal
        // and is expected to run on all projects irrespective of their configuring of the jib
        // plugin).
        ...
        // TODO: this whole sections relies on internal maven API, it could break. We need to
        // explore
        ...
        DependencyFilter ignoreProjectDependenciesFilter =
                (node, parents) -> {
                    ...
                    return Artifact.SCOPE_COMPILE_PLUS_RUNTIME
    ...
    private List<Path> resolveExtraDirectories() throws MojoExecutionException {
        ...
        String deprecatedProperty =
                MavenProjectProperties.getProperty(PropertyNames.EXTRA_DIRECTORY_PATH, project,
                        session);
        String newProperty =
                MavenProjectProperties.getProperty(PropertyNames.EXTRA_DIRECTORIES_PATHS, project,
                        session);
        ...
        if (property != null) {
            ...
            return paths.stream().map(Paths::get).map(Path::toAbsolutePath)
        ...
        return paths.stream().map(File::getAbsoluteFile).map(File::toPath)
                .collect(Collectors.toList());
    }
}
