...
public class MavenProjectPropertiesTest {
    ...
    private static class ContainerBuilderLayers {
        ...
        private ContainerBuilderLayers(BuildConfiguration configuration) {
            ...
            snapshotsLayers =
                    getLayerConfigurationsByName(configuration,
    ...
    private static <T> void assertLayerEntriesUnordered(
            List<T> expectedPaths, List<LayerEntry> entries,
    ...
    private static void assertModificationTime(Instant instant, List<LayerConfiguration> layers) {
        for (LayerConfiguration layer : layers) {
            for (LayerEntry entry : layer.getLayerEntries()) {
                String message =
                        "wrong time: " + entry.getSourceFile() + "-->" + entry.getExtractionPath();
    ...
    @Rule
    public final TestRepository testRepository = new TestRepository();
    @Rule
    ...
    @Mock
    private MavenExecutionRequest mockMavenRequest;
    @Mock
    private Properties mockMavenProperties;
    @Mock
    private Plugin mockJarPlugin;
    @Mock
    private Plugin mockCompilerPlugin;
    @Mock
    private Log mockLog;
    ...
    @Mock
    private Xpp3Dom compilerPluginConfiguration;
    @Mock
    private Xpp3Dom compilerTarget;
    @Mock
    private Xpp3Dom compilerRelease;
    ...
    public void setUp() throws IOException, URISyntaxException {
        ...
        Set<Artifact> artifacts =
                ImmutableSet.of(
                        ...
                        // Maven reads and populates "Artifacts" with its own processing, so read
                        // some from a
                        // repository
    ...
    public void testCreateContainerBuilder_correctFiles()
        ...
        assertSourcePathsUnordered(
                ImmutableList.of(
                        testRepository.artifactPathOnDisk("com.test", "dependencyX",
                                "1.0.0-SNAPSHOT")),
                layers.snapshotsLayers.get(0).getLayerEntries());
        assertSourcePathsUnordered(
                ImmutableList.of(
    ...
    public void testCreateContainerBuilder_warNonDefaultAppRoot()
        ...
        assertSourcePathsUnordered(
                ImmutableList.of(
                        outputPath
                                .resolve("final-name/WEB-INF/lib/dependencyX-1.0.0-SNAPSHOT.jar")),
                layers.snapshotsLayers.get(0).getLayerEntries());
        assertSourcePathsUnordered(
                ImmutableList.of(
